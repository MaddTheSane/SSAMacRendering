...
@interface SubHTMLSpanExtra : NSObject <NSCopying>
{
	@public;
	NSMutableString *str;
}
...
{
	span.extra = [SubHTMLSpanExtra new];
}
...
#define iv() ip = *(int*)p; 
#define sv() sp = *(NSString*__unsafe_unretained*)p;
...
-(void)htmlifyDivArray:(NSArray<SubRenderDiv*>*)divs
{
	for (SubRenderDiv *div in divs) {
		...
		NSInteger spancount = [div->spans count], spans = 1, close_div = 0;
		...
		for (NSInteger j = 0; j < spancount; j++) {
...
int main(int argc, char *argv[])
{
	...
	@autoreleasepool {
		...
		NSURL *fileURL = [[NSURL alloc] initFileURLWithFileSystemRepresentation:argv[1] isDirectory:NO relativeToURL:nil];
		NSString *header = SubLoadSSAFromURL(fileURL, ss);
		ss.finished = YES;
